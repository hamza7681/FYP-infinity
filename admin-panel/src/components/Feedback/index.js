...
const columns = [
  ...
  { id: "added_by", label: "Added By", minWidth: 150 },
  { id: "course", label: "Course Name", minWidth: 200 },
  { id: "rating", label: "Rating", minWidth: 200 },
...
const Feedbacks = () => {
  ...
  const deleteCategory = async () => {
    try {
      const res = await http.delete(`/feedback/delete-feedback/${editId}`, {
  ...
  const handleSearch = (e) => {
    ...
    if (searchTerm === "") {
      ...
      const filterArray = rows.filter((val) => {
        return (
          val.added_by?.firstName
            .toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          val.added_by?.lastName
            .toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          val.rating.toString().toLowerCase().includes(searchTerm.toLowerCase())
        );
      });
      setRows(filterArray);
    }
  ...
  return (
    <div className="pt-[50px] h-screen">
      <div className="bg-[#39405a] py-[10px] flex justify-between items-center rounded-[7px] px-[30px]">
        <h1 className="text-white text-[26px]">Feedbacks</h1>
      </div>
      <DeleteModal
        open={open2}
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell style={{ color: "white" }}>
                            {row.added_by.firstName} {row.added_by.lastName}
                          </TableCell>
                          <TableCell style={{ color: "white" }}>
                            {row.course.title}
                          </TableCell>
